...
from mara.agent.state import SearchWorkerState, SubQuery, SearchResult
...
def _make_state(
    ...
    return SearchWorkerState(
        sub_query=SubQuery(query="test query", domain="test"),
...
class TestFirecrawlScrapeEmptyPaths:
    async def test_empty_search_results_returns_empty_raw_chunks(self, mocker):
        mock_fc = mocker.MagicMock()
        mocker.patch(
            "mara.agent.nodes.search_worker.firecrawl_scrape.Firecrawl",
            return_value=mock_fc,
        )
        result = await firecrawl_scrape(_make_state(search_results=[]), {})
        assert result == {"raw_chunks": []}

    async def test_empty_search_results_does_not_call_batch_scrape(self, mocker):
        mock_fc = mocker.MagicMock()
        mocker.patch(
            "mara.agent.nodes.search_worker.firecrawl_scrape.Firecrawl",
            return_value=mock_fc,
        )
        await firecrawl_scrape(_make_state(search_results=[]), {})
        mock_fc.batch_scrape.assert_not_called()

    async def test_job_data_none_returns_empty_raw_chunks(self, mocker):
    ...
    async def test_doc_with_empty_markdown_is_skipped(self, mocker):
        _mock_firecrawl(mocker, [_make_doc("https://a.com", "")])
        state = _make_state(search_results=[_make_search_result("https://a.com")])
        result = await firecrawl_scrape(state, {})
        assert result["raw_chunks"] == []

    async def test_doc_with_none_markdown_is_skipped(self, mocker):
    ...
    async def test_doc_with_none_metadata_is_skipped(self, mocker):
    ...
    async def test_doc_with_empty_source_url_is_skipped(self, mocker):
...
class TestFirecrawlScrapeDeduplication:
    async def test_duplicate_urls_deduplicated_before_batch_scrape(self, mocker):
    ...
    async def test_deduplication_preserves_first_occurrence_order(self, mocker):
    ...
    async def test_unique_urls_all_passed_to_batch_scrape(self, mocker):
        mock_fc = _mock_firecrawl(mocker, [])
        urls = [f"https://site{i}.com" for i in range(5)]
        state = _make_state(search_results=[_make_search_result(u) for u in urls])
        await firecrawl_scrape(state, {})
        urls_sent = mock_fc.batch_scrape.call_args.args[0]
...
class TestFirecrawlScrapeAPICall:
    async def test_batch_scrape_called_with_markdown_format(self, mocker):
        mock_fc = _mock_firecrawl(mocker, [])
        state = _make_state(search_results=[_make_search_result("https://a.com")])
        await firecrawl_scrape(state, {})
        assert mock_fc.batch_scrape.call_args.kwargs["formats"] == ["markdown"]

    async def test_firecrawl_instantiated_with_api_key(self, mocker):
...
class TestFirecrawlScrapeChunkPopulation:
    async def test_returns_dict_with_raw_chunks_key(self, mocker):
        _mock_firecrawl(mocker, [_make_doc("https://a.com", "hello world content")])
        state = _make_state(search_results=[_make_search_result("https://a.com")])
        result = await firecrawl_scrape(state, {})
        assert "raw_chunks" in result

    async def test_chunk_url_matches_doc_source_url(self, mocker):
    ...
    async def test_chunk_text_contains_source_content(self, mocker):
    ...
    async def test_chunk_sub_query_matches_state_sub_query(self, mocker):
    ...
    async def test_retrieved_at_is_iso8601_utc_format(self, mocker):
        _mock_firecrawl(mocker, [_make_doc("https://a.com", "content")])
        state = _make_state(search_results=[_make_search_result("https://a.com")])
        result = await firecrawl_scrape(state, {})
    ...
    async def test_all_chunks_from_same_call_share_retrieved_at(self, mocker):
    ...
    async def test_chunking_respects_chunk_size_config(self, mocker):
        text = "a" * 500
    ...
    async def test_chunking_respects_chunk_overlap_config(self, mocker):
    ...
    async def test_multiple_docs_all_produce_chunks(self, mocker):
        docs = [
    ...
    async def test_mixed_valid_and_invalid_docs(self, mocker):
