...
class TestContestedFlagging:
    def test_contested_flag_set_on_low_confidence_large_n(self, mocker, make_mara_state):
        ...
        # corroborating >= n_leaves_contested_threshold (default=2) and confidence below low threshold
        claim = _SC("contested claim", 0.30, [], n_leaves=cfg.n_leaves_contested_threshold, corroborating=cfg.n_leaves_contested_threshold)
        ...
        high_contested = _SC("high contested", 0.90, [], n_leaves=cfg.n_leaves_contested_threshold, corroborating=cfg.n_leaves_contested_threshold)
    ...
    def test_contested_flag_set_when_approved_by_human(self, mocker, make_mara_state):
        ...
        claim = _SC("low n_large", 0.30, [], n_leaves=cfg.n_leaves_contested_threshold, corroborating=cfg.n_leaves_contested_threshold)
        result = hitl_checkpoint(make_mara_state(scored_claims=[claim], config=cfg), config={})
        approved = result["human_approved_claims"]
        assert len(approved) == 1
    ...
    def test_high_confidence_claim_not_contested(self, mocker, make_mara_state):
        ...
        claim = _SC("high conf large n", 0.90, [], n_leaves=cfg.n_leaves_contested_threshold, corroborating=cfg.n_leaves_contested_threshold)
        result = hitl_checkpoint(make_mara_state(scored_claims=[claim], config=cfg), config={})
