...
def _scored_claim(text: str, confidence: float = 0.30, n_leaves: int = 5) -> ScoredClaim:
...
class TestNoFailingClaims:
    ...
    async def test_contested_claims_not_treated_as_failing(self, mocker, make_mara_state):
        # n_leaves >= n_leaves_contested_threshold → not a failing claim
        cfg = ResearchConfig(leaf_db_enabled=False)
        claims = [_scored_claim("contested", confidence=0.30, n_leaves=cfg.n_leaves_contested_threshold)]
