...
class TestFormatLeaves:
    ...
    def test_extracts_index_url_text(self, make_merkle_leaf):
        leaf = make_merkle_leaf(index=3, url="https://a.com", text="some text")
        result = _format_leaves([leaf])
        assert result == [(3, "https://a.com", "some text")]

    def test_preserves_order(self, make_merkle_leaf):
        leaves = [make_merkle_leaf(index=i, url=f"https://a.com/{i}", text=f"t{i}") for i in range(3)]
        result = _format_leaves(leaves)
        assert [r[0] for r in result] == [0, 1, 2]

    def test_uses_leaf_index_not_list_position(self, make_merkle_leaf):
        # leaf has index=7 but is at list position 0
        leaf = make_merkle_leaf(index=7, url="https://a.com", text="text")
        result = _format_leaves([leaf])
        assert result[0][0] == 7

    def test_multiple_leaves(self, make_merkle_leaf):
        leaves = [
            make_merkle_leaf(index=0, url="https://a.com", text="alpha"),
            make_merkle_leaf(index=1, url="https://b.com", text="beta"),
        ]
        result = _format_leaves(leaves)
        assert len(result) == 2
...
class TestClaimExtractorNode:
    ...
    async def test_empty_leaves_returns_empty_claims(self, make_mara_state):
        result = await claim_extractor(make_mara_state(retrieved_leaves=[]), config={})
        assert result == {"extracted_claims": []}

    async def test_empty_leaves_does_not_call_llm(self, mocker, make_mara_state):
        mock_cls = mocker.patch("mara.agent.nodes.claim_extractor.make_llm")
        await claim_extractor(make_mara_state(retrieved_leaves=[]), config={})
        mock_cls.assert_not_called()

    async def test_returns_extracted_claims_key(self, mocker, make_mara_state, make_merkle_leaf):
        leaf = make_merkle_leaf(index=0)
        self._mock_llm(mocker, _claims_json())
        result = await claim_extractor(make_mara_state(retrieved_leaves=[leaf]), config={})
        assert "extracted_claims" in result

    async def test_claims_are_list_of_claim(self, mocker, make_mara_state, make_merkle_leaf):
        leaf = make_merkle_leaf(index=0)
        self._mock_llm(mocker, _claims_json())
        result = await claim_extractor(make_mara_state(retrieved_leaves=[leaf]), config={})
    ...
    async def test_claim_count_matches_llm_response(self, mocker, make_mara_state, make_merkle_leaf):
        leaf = make_merkle_leaf(index=0)
    ...
    async def test_llm_invoked_with_system_and_user_messages(self, mocker, make_mara_state, make_merkle_leaf):
        leaf = make_merkle_leaf(index=0)
    ...
    async def test_system_message_contains_system_prompt(self, mocker, make_mara_state, make_merkle_leaf):
        leaf = make_merkle_leaf(index=0)
        mock_llm = self._mock_llm(mocker, _claims_json())
        await claim_extractor(make_mara_state(retrieved_leaves=[leaf]), config={})
        messages = mock_llm.ainvoke.call_args.args[0]
    ...
    async def test_user_message_contains_leaf_url(self, mocker, make_mara_state, make_merkle_leaf):
        leaf = make_merkle_leaf(index=0, url="https://distinctive-url.com")
        mock_llm = self._mock_llm(mocker, _claims_json())
        await claim_extractor(make_mara_state(retrieved_leaves=[leaf]), config={})
        messages = mock_llm.ainvoke.call_args.args[0]
        assert "https://distinctive-url.com" in messages[1]["content"]

    async def test_user_message_contains_leaf_index(self, mocker, make_mara_state, make_merkle_leaf):
        leaf = make_merkle_leaf(index=5)
        mock_llm = self._mock_llm(mocker, _claims_json())
        await claim_extractor(make_mara_state(retrieved_leaves=[leaf]), config={})
        messages = mock_llm.ainvoke.call_args.args[0]
    ...
    async def test_config_forwarded_to_ainvoke(self, mocker, make_mara_state, make_merkle_leaf):
        ...
        await claim_extractor(make_mara_state(retrieved_leaves=[leaf]), config=sentinel_config)
        _, forwarded_config = mock_llm.ainvoke.call_args.args
        assert forwarded_config is sentinel_config

    async def test_uses_config_model_and_api_key(self, mocker, make_mara_state, make_merkle_leaf):
    ...
    async def test_handles_empty_claim_list_from_llm(self, mocker, make_mara_state, make_merkle_leaf):
        leaf = make_merkle_leaf(index=0)
        self._mock_llm(mocker, "[]")
        result = await claim_extractor(make_mara_state(retrieved_leaves=[leaf]), config={})
        assert result["extracted_claims"] == []

    async def test_multiple_leaves_all_passed_to_llm(self, mocker, make_mara_state, make_merkle_leaf):
        leaves = [make_merkle_leaf(index=i, url=f"https://a.com/{i}", text=f"text {i}") for i in range(3)]
        mock_llm = self._mock_llm(mocker, _claims_json())
        await claim_extractor(make_mara_state(retrieved_leaves=leaves), config={})
