...
def get_embedding_model(model_name: str, token: str = "") -> SentenceTransformer:
    ...
    Args:
        ...
        token:      Optional HuggingFace Hub token for authenticated downloads.
    """
...
def embed(texts: list[str], model_name: str, token: str = "") -> np.ndarray:
    ...
    """
    model = get_embedding_model(model_name, token=token)
